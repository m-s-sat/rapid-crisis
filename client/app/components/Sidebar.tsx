...
const navItems = [
  { name: "Command Center", path: "/dashboard", icon: "CC" },
  { name: "Staff Directory", path: "/management/staff", icon: "ST" },
  { name: "Guest Registry", path: "/management/guests", icon: "GU" },
  { name: "System Settings", path: "/settings", icon: "SE" },
];
  ...
  <>
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link key={item.path} href={item.path} className="no-underline">
            ...
            >
              <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-muted text-[0.6rem] font-bold">
                {item.icon}
              </span>
              <span>{item.name}</span>
              {isActive && (
    ...
    <div className="px-1 pb-2">
      <div className="mb-3 group cursor-default">
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-b from-sidebar-accent/50 to-sidebar-accent/10 p-4 border border-sidebar-border/20 shadow-sm transition-all duration-300 group-hover:border-primary/30">
